...
public class LayerCape implements LayerRenderer<AbstractClientPlayer>
{
    ...
    public void doRenderLayer(AbstractClientPlayer entitylivingbaseIn, float p_177141_2_, float p_177141_3_, float partialTicks, float p_177141_5_, float p_177141_6_, float p_177141_7_, float scale)
    {
        if (!entitylivingbaseIn.isInvisible() && (entitylivingbaseIn.hasPlayerInfo() && entitylivingbaseIn.isWearing(EnumPlayerModelParts.CAPE) && entitylivingbaseIn.getLocationCape() != null || entitylivingbaseIn.getName().equals(Minecraft.getMinecraft().getSession().getUsername()) && Demise.INSTANCE.getModuleManager().getModule(Interface.class).isEnabled() && Demise.INSTANCE.getModuleManager().getModule(Interface.class).cape.get()))        {
            ...
            if(entitylivingbaseIn.getName().equals(Minecraft.getMinecraft().getSession().getUsername()) && Demise.INSTANCE.getModuleManager().getModule(Interface.class).cape.get()){
            ...
            if(entitylivingbaseIn.getName().equals(Minecraft.getMinecraft().getSession().getUsername()) && Demise.INSTANCE.getModuleManager().getModule(Interface.class).cape.get()){
                ...
                if (Demise.INSTANCE.getModuleManager().getModule(Interface.class).enchanted.get()){
