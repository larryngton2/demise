...
public abstract class World implements IBlockAccess {
    ...
    private int getRawLight(BlockPos pos, EnumSkyBlock skyBlock) {
        if (skyBlock == EnumSkyBlock.SKY && this.canSeeSky(pos)) {
            return 15;
        } else {
            Block block = this.getBlockState(pos).getBlock();
            ...
            if (i < 14) {
                for (EnumFacing enumfacing : EnumFacing.values()) {
                    ...
                    int k = this.getLightFor(skyBlock, blockpos) - j;
                    if (k > i) {
                        i = k;
                    }
    ...
    public boolean checkLightFor(EnumSkyBlock lightType, BlockPos pos) {
        if (!this.isAreaLoaded(pos, 17, false)) {
            ...
            // this is so the client doesn't run slower than augustus
            EXECUTOR.execute(() -> {
                int i = 0;
                int j = 0;
