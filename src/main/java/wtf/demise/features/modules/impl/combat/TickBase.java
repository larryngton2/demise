...
public class TickBase extends Module {
    ...
    public void onTimerManipulation(TimerManipulationEvent e) {
        ...
        if (shouldStart() && timer.hasTimeElapsed(delay.get())) {
            if (target.hurtTime == 10) {
                DebugUtils.sendMessage("HitBug");
            }

            shifted += e.getTime() - previousTime;
        }

        if (shifted >= maxTick.get() * (1000 / 20f) || !shouldStart()) {
            shifted = 0;
            timer.reset();
        }
    ...
    public void onRender3D(Render3DEvent e) {
        ...
        if (renderPredictedTargetPos.get()) {
            Vec3 prediction = target.getPositionVector().subtract(new Vec3(target.prevPosX, target.prevPosY, target.prevPosZ)).multiply(targetOffset.get());
