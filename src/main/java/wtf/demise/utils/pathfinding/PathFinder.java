...
public class PathFinder implements InstanceAccess {
    ...
    public static boolean isValid(final Vec3 loc, final boolean checkGround) {
        return isValid((int) loc.xCoord, (int) loc.yCoord, (int) loc.zCoord,
                checkGround);
    }
    ...
    public PathHub doesHubExistAt(final Vec3 loc) {
        for (final PathHub pathHub : this.pathHubs) {
            if (pathHub.getLoc().xCoord != loc.xCoord || pathHub.getLoc().yCoord != loc.yCoord
                    || pathHub.getLoc().zCoord != loc.zCoord) {
                continue;
            }
            return pathHub;
        }
    ...
    public boolean putHub(final PathHub parent, final Vec3 loc, final double cost) {
        ...
        if (existingPathHub == null) {
            if (loc.xCoord == this.endVec3Path.xCoord && loc.yCoord == this.endVec3Path.yCoord
                    && loc.zCoord == this.endVec3Path.zCoord
