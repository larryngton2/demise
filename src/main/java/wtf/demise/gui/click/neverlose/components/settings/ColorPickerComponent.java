package wtf.demise.gui.click.neverlose.components.settings;
...
import static wtf.demise.gui.click.neverlose.NeverLose.lineColor2;
import static wtf.demise.gui.click.neverlose.NeverLose.textRGB;
...
public class ColorPickerComponent extends Component {
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) {
        if (MouseUtils.isHovered(getX() + 157, getY() + 15f, 14, 14, mouseX, mouseY) && mouseButton == 1) {
            opened = !opened;
        }
        if (opened) {
            if (mouseButton == 0) {
                if (MouseUtils.isHovered(getX() + 78, getY() + 34, 8, 78, mouseX, mouseY)) {
                    pickingHue = true;
                }
                if (MouseUtils.isHovered(getX() + 90, getY() + 34, 80, 80, mouseX, mouseY)) {
                    picking = true;
                }
                if (MouseUtils.isHovered(getX() + 65, getY() + 34, 8, 78, mouseX, mouseY)) {
                    pickingAlpha = true;
                }
            }
        }
