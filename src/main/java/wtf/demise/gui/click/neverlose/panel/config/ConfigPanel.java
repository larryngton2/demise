    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        if (configMap.keySet().stream().filter(ConfigRect::isSelected).findAny().orElse(null) == null) {
            if (!configMap.keySet().stream().toList().isEmpty()) {
                Objects.requireNonNull(configMap.keySet().stream().filter(configRect -> configRect.getConfig().getName().equals(Demise.INSTANCE.getConfigManager().getCurrentConfig())).findFirst().orElse(null)).setSelected(true);
            }
        }
        //update coordinate
